...
static void operador_1(struct svc_req *rqstp, SVCXPRT *transp);

static void
operador_1(struct svc_req *rqstp, SVCXPRT *transp)
{
	union {
	...
	case SUMA:
		xdr_argument = (xdrproc_t) xdr_suma_1_argument;
		xdr_result = (xdrproc_t) xdr_int;
		local = (char *(*)(char *, struct svc_req *)) _suma_1;
		break;

	case RESTA:
		xdr_argument = (xdrproc_t) xdr_resta_1_argument;
		xdr_result = (xdrproc_t) xdr_int;
		local = (char *(*)(char *, struct svc_req *)) _resta_1;
		break;

	case MULTIPLICACION:
		xdr_argument = (xdrproc_t) xdr_multiplicacion_1_argument;
		xdr_result = (xdrproc_t) xdr_int;
		local = (char *(*)(char *, struct svc_req *)) _multiplicacion_1;
		break;

	case DIVISION:
		xdr_argument = (xdrproc_t) xdr_division_1_argument;
		xdr_result = (xdrproc_t) xdr_int;
...
main( int argc, char* argv[] )
{
	...
	if (getsockname(0, (struct sockaddr *)&saddr, &asize) == 0) {
		...
		_rpcpmstart = 1;
		proto = 0;
		openlog("operaciones_aritmeticas", LOG_PID, LOG_DAEMON);
	} else {
#ifndef RPC_SVC_FG
		int size;
		int pid, i;
...
#endif
		sock = RPC_ANYSOCK;
		(void) pmap_unset(OPERADOR, ARITMETICAS);
	}
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_DGRAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_UDP;
		if (!svc_register(transp, OPERADOR, ARITMETICAS, operador_1, proto)) {
			_msgout("unable to register (OPERADOR, ARITMETICAS, udp).");
			exit(1);
		}
	}
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_STREAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_TCP;
		if (!svc_register(transp, OPERADOR, ARITMETICAS, operador_1, proto)) {
			_msgout("unable to register (OPERADOR, ARITMETICAS, tcp).");
			exit(1);
		}
	}
